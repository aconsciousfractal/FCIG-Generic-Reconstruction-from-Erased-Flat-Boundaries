...
whose two comparison conditions are each provably necessary, and a
16,744-instance finite replay: reconstruction on every instance, exact
certification on a deterministic 340-instance sample.  The finite replay
supports the implementation; it is not used as the proof.
\end{abstract}
...
boundary data.  Aurenhammer takes a supplied polytopical cell
...
Several further inverse-tessellation methods reinforce this distinction.
Suzuki and Iri approximate a supplied planar tessellation by a Voronoi diagram,
with exact generator restoration included as a special case
\cite{SuzukiIri1986}.  Duan et al. recover weighted generators from a supplied
Laguerre tessellation and show that those generators are generally nonunique
\cite{DuanEtAl2014}.  Chaidee and Sugihara recognise spherical Laguerre
Voronoi diagrams from a supplied spherical tessellation
\cite{ChaideeSugihara2017}.  Most recently, Hernandez-Suarez reconstructs the
generators of a supplied planar Voronoi tessellation from one interior cell and
its neighbours, then propagates them by successive reflections
\cite{HernandezSuarez2025}.  In every case the diagram, partition, or
tessellation boundaries remain part of the input.
